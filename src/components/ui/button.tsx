...
const classes = {
    root: 'inline-flex items-center justify-center shrink-0 leading-none rounded outline-none transition duration-300 ease-in-out focus:outline-none focus:shadow focus:ring-1 focus:ring-accent-700',
    normal: 'bg-cta-blue text-light border border-transparent hover:bg-accent-hover',
    custom: 'border border-transparent bg-cyan-650 hover:bg-light-green text-light',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>((props, ref) => {
    ...
    return (
        <button
            ...
            {...rest}
        >
            {add && <AddIcon circleFill={ven ? '#273E61' : '#019376'} rectFill="#FFF" />}
            {children}
            {next && <ArrowNext color={disabled ? 'gray' : '#FFF'} />}
            {loading && (
                <span
